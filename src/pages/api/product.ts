...
export default async function handler(
  ...
  const response = await client.getEntries({
    ...
    'fields.handle[in]': req.query.search,
    include: 10,
  });
  ...
  const getProductVariants = (product) => {
    return product.fields.productVariants.map((variant) => ({
      ...
      stripePriceId: variant.fields.stripePriceId,
      price: {
...
//   const product = response.items.map( item => item.fields.products)
  const product = response.items.map(
    (product) => ({
      id: product.sys.id,
